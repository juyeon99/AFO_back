...
public class JwtProvider {
    ...
    public JwtProvider(@Value("${jwt.secret_key") String keyParam,
        ...
        key = keyParam;
        keyBase64Encoded = Base64.getEncoder().encodingToString(key.getBytes());
    ...
    /**
     * @return JwtToken(액세스 토큰 + 리프레시 토큰)
     */
    public JwtToken createJwtToken(Long memberId, String nickname, Boolean isAdmin) {
        String accessToken = createAceessToken(memberId, nickname, isAdmin);
    ...
    /**
     * 리프레시 토큰으로 랙세스 토큰 재발급 요청 왔을대 호출됨
     *
     * @reaturn 액세스 토큰
     */
    public String createAceessToken(Long memberId, String nickname, Boolean isAdmin) {
        ...
        String accessToken = ACCESS_PREFIX_STRING + JwtClaimsSet.builder()
     ...
     * 토큰 클레임들 가져오기
     *
     * @param 액세스토큰 또는 리프레쉬토큰
     * @return 해당 토큰의 클레임들, parse 실패시 null 반환
     */
